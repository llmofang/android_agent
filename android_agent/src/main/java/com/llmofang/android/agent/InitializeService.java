...
public class InitializeService implements Runnable {
    ...
    public void run() {
        ...
        LLMoFang.flow = LLMoFang.flow / 1024/1024;
//        ApplicationInfo info = LLMoFang.applicationContext.getApplicationInfo();
//        long todayFlow = TrafficStats.getUidRxBytes(info.uid);
//        todayFlow = todayFlow + TrafficStats.getUidTxBytes(info.uid);
        LLMoFangUtil.showToast("流量魔方初始化成功,可用流量为" + LLMoFang.flow + "Mb");
...
//                .addHeader("X-llmf-REST-API-Key",appKey).build();
        ...
        try {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                LLMoFang.apptokenExpire = LLMoFangUtil.getExpireTime(result.getInt("expire") - 60);
    ...
    public void acquireInitData() {
        ...
        if (LLMoFangUtil.isExpire(LLMoFang.apptokenExpire)) {
            this.acquireAppToken();
        }
...
//            response = client.newCall(request).execute();
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                LLMoFang.requestTokenExpire = LLMoFangUtil.getExpireTime(result.getInt("expire") - 60);
                ...
                LLMoFang.initialErrorRetry = errorPolicy.getInt("retry");
